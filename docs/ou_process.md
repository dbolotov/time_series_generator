...
Create a sequence using the [Ornstein–Uhlenbeck (OU) Process](https://en.wikipedia.org/wiki/Ornstein%E2%80%93Uhlenbeck_process) - a mean-reverting stochastic process. 
...
Common use cases include modeling interest rates (e.g. [Vasicek model](https://en.wikipedia.org/wiki/Vasicek_model)), physical systems with friction (e.g. velocity of a particle in a fluid), or any process where random shocks are tempered by a stabilizing force.